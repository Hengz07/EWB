<?php
	session_start();
		...
		if(isset($_POST['submit']))
		{
			...
			$query = "UPDATE single_score set point=$point, ob=$ob where single_score_id = $singlescoreId";
			$updateScore = mysqli_query($con, $query);
			
			if ($updateScore === TRUE) {
				
				$querySinglePlayer = "select * from singel_palyer where id = $singlePlayerId limit 1";
				...
				header("Location: result.php?check=".$singlescoreId);
					
				}
				else{
					
				$queryid = "select * from single_score where single_score_id =$singlescoreId limit 1";
